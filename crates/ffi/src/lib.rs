//! Gõ Nhanh FFI Library
//!
//! This crate provides FFI bindings for integrating the Gõ Nhanh
//! engine with platform-specific code (iOS, Android, Windows, Linux, macOS).
//!
//! # FFI Usage
//!
//! ```c
...
// Re-export updater FFI functions so they're included in the static library.
// Without this, the linker dead-strips them since ffi doesn't call them directly.
pub use updater::{version_compare, version_has_update};
...
pub unsafe extern "C" fn ime_add_shortcut(
    ...
    if let Some(ref mut e) = *guard {
        ...
        let shortcut = if is_symbol_trigger {
            engine::Shortcut::immediate(trigger_str, replacement_str)
        } else {
            engine::Shortcut::new(trigger_str, replacement_str)
        };
...
mod tests {
    ...
    fn test_shortcut_ffi_symbol_trigger_immediate() {
        ...
        if let Some(ref e) = *guard {
            ...
            assert_eq!(
                shortcut.condition,
                engine::TriggerCondition::Immediate,
    ...
    fn test_shortcut_ffi_letter_trigger_word_boundary() {
        ...
        if let Some(ref e) = *guard {
            ...
            assert_eq!(
                shortcut.condition,
                engine::TriggerCondition::OnWordBoundary,
